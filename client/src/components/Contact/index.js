        ...
        <div className="main-form">
          ...
          <form onSubmit={handleSubmit(onSubmit)}>
            <label htmlFor="name">Nom
              <input name="name" ref={register({ required: true })} type="text" defaultValue={user ? `${user.first_name} ${user.last_name}` : ''} />
            </label>
            {errors.name && <span className="warning-text">Veuillez saisir votre nom</span>}
            <label htmlFor="email">Email
              <input name="email" ref={register({ required: true })} type="email" defaultValue={user ? user.email : ''} />
            </label>
